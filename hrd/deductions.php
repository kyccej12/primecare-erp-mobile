				...
				$.post("misc-data.php", { mod: "getDeduction", id: UID, sid: Math.random() }, function(ech) {
					...
					$("#record").dialog({title: "Record Details", width: 440, resizable: false, modal: true, buttons: { 
						"Save Changes": function() {
							$.post("misc-data.php", { mod: "updateDeduction", rid: $("#rid").val(), emp_id: $("#emp_id").val(), type: $("#deduction_type").val(), period: $("#pay_period").val(), amount: $("#amount").val(), doc_no: $("#doc_no").val(), doc_date: $("#doc_date").val(), doc_type: $("#doc_type").val(), remarks: $("#remarks").val(), sid: Math.random() }, function() {
			...
			$("#record").dialog({title: "New Record", width: 440, resizable: false, modal: true, buttons: {
				"Save Record": function() {
					...
					if(msg != "") { parent.sendErrorMessage(msg); } else {
						$.post("misc-data.php", { mod: "newDeduction", emp_id: $("#emp_id").val(), type: $("#deduction_type").val(), period: $("#pay_period").val(), amount: $("#amount").val(), doc_no: $("#doc_no").val(), doc_date: $("#doc_date").val(), doc_type: $("#doc_type").val(), remarks: $("#remarks").val(), sid: Math.random() }, function() {
			...
			<table width=100% cellspacing=2 cellpadding=0>
				<tr>
					...
					<td align=left>
						...
						<select class="gridInput" name="emp_id" id="emp_id" style="width: 80%; font-size: 11px;">
							<?php
				...
				<tr>
					...
					<td align=left>
						<select name="deduction_type" id="deduction_type" class="gridInput" style="width: 80%; font-size: 11px;">
							<option value="">- Select Type -</option>
							<?php 
								$d_res = $mydb->dbquery("select id, deduction_type from option_deductiontype order by deduction_type asc;");
				...
				<tr>
					...
					<td align=left>
						<select class="gridInput" name="pay_period" id="pay_period" style="width: 80%; font-size: 11px;">
							<?php
				...
				<tr>
					<td class="bareThin" align=left width=40%>Ref. # :</td>
					<td align=left>
				...
				<tr>
					...
					<td align=left>
						<select class="gridInput" name="doc_type" id="doc_type" style="width: 80%; font-size: 11px;">
							...
							<option vallue='JV'>Journal Voucher</option>
							<option value='CV'>Check Voucher</option>
							<option value='DA'>Debit/Credit Advise</option>
							<option value='OT'>Others (See Remarks)</option>
						</select>
					</td>
				</tr>
				<tr>
