					...
					$("#record").dialog({title: "Record Details", width: 440, resizable: false, modal: true, buttons: { 
						"Save Changes": function() {
							...
							if(msg != '') {
								...
								$.post("misc-data.php", { mod: "updateAdjustment", rid: $("#rid").val(), emp_id: $("#emp_id").val(), type: $("#type").val(), period: $("#pay_period").val(), amount: $("#amount").val(), taxable: $("#is_taxable").val(), remarks: $("#remarks").val(), sid: Math.random() }, function() {
			...
			$("#record").dialog({title: "New Record", width: 440, resizable: false, modal: true, buttons: {
				"Save Record": function() {
					...
					if(msg != "") { parent.sendErrorMessage(msg); } else {
						$.post("misc-data.php", { mod: "newAdjustment", emp_id: $("#emp_id").val(), type: $("#type").val(), period: $("#pay_period").val(), amount: $("#amount").val(), taxable: $("#is_taxable").val(), remarks: $("#remarks").val(), sid: Math.random() }, function() {
		...
		<form name="irec" id="irec">
			<table width=100% cellspacing=2 cellpadding=0>
				...
				<tr>
					...
					<td align=left>
						<select class="gridInput" name="type" id="type" style="width: 80%; font-size: 11px;">
							<?php
								$atQuery = $mydb->dbquery("select id, adjust_type from option_adjustments order by adjust_type;");
								while($atRow = $atQuery->fetch_array()) {
									echo "<option value='$atRow[0]'>$atRow[1]</option>";
								}
							?>
						</select>
					</td>
				...
				<tr>
					...
					<td align=left>
						<input type="hidden" name="rid" id="rid" value="">
						<select class="gridInput" name="is_taxable" id="is_taxable" style="width: 80%; font-size: 11px;">
