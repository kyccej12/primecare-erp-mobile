...
/* MYSQL QUERIES SECTION */
	...
	$mpdf=new mPDF('win-1252','LETTER','','',10,10,50,10,10,10);
	...
	if($_GET['dtf'] != '') { $fs = "  AND c.extractdate BETWEEN '$dtf' AND '$dt2' "; } 
	if($_GET['type'] != '') { $fs = " and d.result_stat = '$_GET[type]' "; }

	switch($_REQUEST['sort']) {
		case "1": default:
			$order = " ORDER BY b.lname,b.fname,b.mname";
		break;
		case "2":
			$order = " ORDER BY c.lotno asc";
		break;
		case "3":
			$order = " ORDER BY b.employer";
		break;
	}
	...
	$outerQuery = $con->dbquery("SELECT a.pid, CONCAT(b.lname,', ',b.fname,' ',b.mname) AS pname, b.birthdate, DATE_FORMAT(b.birthdate,'%d %b %Y') AS bday, IF(b.gender='M','Male','Female') AS gender, b.street, b.brgy, b.city, b.province, b.gender AS xgender, a.barcode, c.extractdate, d.result_type, verified FROM cso_details a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN lab_samples c ON a.pid = c.pid LEFT JOIN lab_descriptive d ON a.pid = d.pid AND a.cso_no = d.so_no AND c.serialno = d.serialno WHERE a.cso_no = '$_REQUEST[so_no]' AND c.`code` LIKE '%X%' $fs $order");
	while($outerRow = $outerQuery->fetch_array()) {
		...
		if($comp == '185') {
			...
			list($employer) = $con->getArray("select company from cso_header where cso_no = '$comp';");
		}
