...
/* MYSQL QUERIES SECTION */
	...
	list($cname,$soDate,$resultDate) = $con->getArray("select customer_name,cso_date, date_format(until,'%d %b %Y') from cso_header where cso_no = '$_REQUEST[so_no]';");

	//$outerQuery = $con->dbquery("SELECT a.pid, CONCAT(b.lname,', ',b.fname,' ',b.mname) AS pname, b.birthdate, DATE_FORMAT(b.birthdate,'%d %b %Y') AS bday, IF(b.gender='M','Male','Female') AS gender, b.street, b.brgy, b.city, b.province, b.gender AS xgender, a.barcode, DATE_FORMAT(c.extractdate,'%m/%d/%Y') AS extractdate,b.employer FROM cso_details a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN lab_samples c ON a.pid = c.pid WHERE a.cso_no = '6' AND a.barcode != 'N' AND c.extractdate BETWEEN '$dtf' AND '$dt2' AND b.employer= 'ADVANCED WORLD SYSTEMS, INC.' GROUP BY a.pid ORDER BY b.lname DESC, b.fname, b.mname;");
	$outerQuery = $con->dbquery("SELECT a.pid, CONCAT(b.lname,', ',b.fname,' ',b.mname) AS pname, b.birthdate, DATE_FORMAT(FROM_DAYS(DATEDIFF(d.cso_date,b.birthdate)), '%Y') + 0 AS age, DATE_FORMAT(b.birthdate,'%d %b %Y') AS bday, IF(b.gender='M','Male','Female') AS gender, b.street, b.brgy, b.city, b.province, b.gender AS xgender, a.barcode, DATE_FORMAT(c.extractdate,'%m/%d/%Y') AS extractdate,b.employer FROM cso_details a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN lab_samples c ON a.pid = c.pid LEFT JOIN cso_header d ON a.cso_no = d.cso_no WHERE a.cso_no = '6' AND a.barcode != 'N' AND c.extractdate BETWEEN '$dtf' AND '$dtf' AND b.employer= 'ADVANCED WORLD SYSTEMS, INC.' GROUP BY a.pid ORDER BY b.lname DESC, b.fname, b.mname;");
			...
			<html>
				...
				<htmlpageheader name="myheader">
					...
					<table width=100% cellpadding=2 cellspacing=0 style="font-size:8pt;margin-top:5px;">
						...
						<tr>
							<td><b>COMPANY</b></td>
							<td>:&nbsp;&nbsp;' . $outerRow['employer'] . '</td>
							<td></td>
							<td></td>
							<td><b>RESULT DATE</b></td>
							<td>:&nbsp;&nbsp;' . $outerRow['extractdate'] . '</td>
						</tr>
						<tr>
				...
				<htmlpagefooter name="myfooter">
					<table width=100% cellpadding=5 style="margin-top: 10px; font-size: 6pt;">
						<tr>
