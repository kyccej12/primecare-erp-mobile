...
/* MYSQL QUERIES SECTION */
	...
	$mpdf=new mPDF('win-1252','LETTER','','',8,8,60,5,5,5);
    ...
    $outerRow = $con->getArray("SELECT distinct a.pid, b.patient_id, CONCAT(b.lname,', ',b.fname,' ',b.mname) AS pname, b.birthdate, DATE_FORMAT(b.birthdate,'%d %b %Y') AS bday, DATE_FORMAT(FROM_DAYS(DATEDIFF(c.extractdate,b.birthdate)), '%Y') + 0 AS age, IF(b.gender='M','Male','Female') AS gender, b.street, b.brgy, b.city, b.province, b.gender AS xgender, a.barcode, DATE_FORMAT(c.extractdate,'%m/%d/%Y') AS extractdate,  b.employer, a.trace_no FROM cso_details a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN lab_samples c ON a.pid = c.pid WHERE a.cso_no = '$_REQUEST[so_no]' AND a.pid = '$_REQUEST[pid]' GROUP BY a.pid limit 1;");

	//list($employer) = $con->getArray("select employer from pccmain.patient_info where patient_id = '$outerRow[patient_id]';");

	$comp = $_REQUEST['so_no'];
	$employer = '';
	if($comp == '208') {
		$employer = "SPEEDY TIRES INCORPORATED";
	}else {
		list($employer) = $con->getArray("select employer from pccmain.patient_info where patient_id = '$outerRow[pid]';");
	}

	list($extractDateNonXray) = $con->getArray("select date_format(extractdate,'%m/%d/%Y') from lab_samples where so_no = '$_REQUEST[so_no]' and pid = '$outerRow[pid]' and `code` = 'L010' GROUP BY so_no, pid;");
	...
	*/
	...
	$cbcResult = $con->getArray("SELECT *,machine,date_format(result_date,'%m/%d/%Y') as rdate FROM lab_cbcresult WHERE serialno = '$cbcSN';");
	...
	$casts = '';
	if($uaResult['hyaline'] != '') { 
		$casts .= "HYALINE: " . $uaResult['hyaline'] . "<br/>";
	}
	...
	$html = '
		<html>
			...
			<htmlpagefooter name="myfooter">
				<table width=100%>
					<tr><td align=left><barcode size=0.8 code="'.substr($outerRow['trace_no'],0,10).'" type="C128A"></td><td align=right>Date & Time Printed: '.date('m/d/Y h:i:s a').'</td></tr>
				</table>
			</htmlpagefooter>
				...
				$html .= '</td>
					<td width=50% valign=top>';
						...
						if(count($otherResult) > 0) {
							...
							$html .= '<table width=100% cellpadding=0 align=center style="font-size: 8pt;"> 
								<tr>
