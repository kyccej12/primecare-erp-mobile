...
/* MYSQL QUERIES SECTION */
	...
	// if($_GET['shift'] == 1) {
	// 	$fs = " and a.processed_on between '$date 06:00:00' and '$date 14:00:00' ";
	// } else {
	// 	list($nextday) = $con->getArray("SELECT DATE_ADD('$date', INTERVAL 1 DAY);");
	// 	$fs = " and a.processed_on between '$date 20:00:00' and '$nextday 05:59:00' ";
	// }
	...
	if($_GET['date'] != '') {
		...
		if($_GET['shift'] != '') {
			if($_GET['shift'] == 1) {
				$range = " AND processed_on BETWEEN '$date 18:00:00' and '$date 05:59:00' ";
			} else {
				$range = " AND processed_on BETWEEN '$date 06:00:01' and '$date 17:59:00' ";
			}
		} else {
			$xso = $_REQUEST['so_no'] * 1;
			$shiftedCSO = array(78);
			if(in_array($xso,$shiftedCSO)) {
				$range = " AND processed_on BETWEEN '$date 19:00:00' and '$nextday 12:59:00' ";
			} else {
	...
	$outerQuery = $con->dbquery("SELECT a.pid, CONCAT(b.lname,', ',b.fname,' ',b.mname) AS pname, b.birthdate, date_format(b.birthdate,'%d %b %Y') as bday, if(b.gender='M','Male','Female') as gender, b.street, b.brgy, b.city, b.province, b.gender as xgender, a.barcode, DATE_FORMAT(c.extractdate,'%m/%d/%Y') AS extractdate FROM cso_details a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN lab_samples c on a.pid = c.pid WHERE a.cso_no = '$_REQUEST[so_no]' and a.barcode != 'N' $searchString GROUP BY a.pid ORDER BY b.lname desc, b.fname, b.mname;");
	...
	while($outerRow = $outerQuery->fetch_array()) {
		...
		list($employer_ad) = $con->getArray("select employer from pccmain.patient_info where patient_id = '$outerRow[pid]';");
