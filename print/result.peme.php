<?php
	...
	//$_ihead = $con->getArray("SELECT a.*, LPAD(so_no,6,0) AS sono, DATE_FORMAT(so_date,'%m/%d/%Y') AS d8, DATE_FORMAT(a.updated_on, '%m/%d/%Y') AS examin_d8, b.patient_id, fname, b.lname, b.mname,FLOOR(DATEDIFF(so_date,b.birthdate)/364.25) AS age,DATE_FORMAT(b.birthdate,'%M %d') AS date1, DATE_FORMAT(b.birthdate,'%Y') AS date2, classification,examined_by,b.gender,b.brgy,b.city,b.province,b.street,c.civil_status,b.birthplace,b.employer FROM peme a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN options_civilstatus c ON b.cstat = c.csid WHERE a.so_no= '$_REQUEST[so_no]' AND a.pid= '$_REQUEST[pid]';");
	$_ihead = $con->getArray("SELECT a.*, LPAD(so_no,6,0) AS sono, DATE_FORMAT(so_date,'%m/%d/%Y') AS d8, DATE_FORMAT(a.updated_on, '%m/%d/%Y') AS examin_d8, b.patient_id, fname, b.lname, b.mname,FLOOR(DATEDIFF(so_date,b.birthdate)/364.25) AS age,DATE_FORMAT(b.birthdate,'%M %d') AS date1, DATE_FORMAT(b.birthdate,'%Y') AS date2, classification,examined_by,b.gender,b.brgy,b.city,b.province,b.street,c.civil_status,b.birthplace,b.employer, DATE_FORMAT(d.processed_on,'%m/%d/%Y') AS d8 FROM peme a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN options_civilstatus c ON b.cstat = c.csid LEFT JOIN cso_details d ON a.pid = d.pid WHERE a.so_no= '$_REQUEST[so_no]' AND a.pid= '$_REQUEST[pid]';");
	...
	list($resultDatex) = $con->getArray("select date_format(processed_on,'%m/%d/%Y') from cso_details where cso_no = '$_ihead[so_no]' and pid = '$_REQUEST[pid]';");

	list($cname,$soDate,$resultDate) = $con->getArray("select company,cso_date, date_format(until,'%d %b %Y') from cso_header where cso_no = '$_ihead[so_no]';");


	/* Customize SO# -> employer concern for printing */
	$comp = $_REQUEST['so_no'];
		$employer = '';
		if($comp == '194') {
			$employer = "CONTACTPOINT360 PH";
		}else {
			list($employer) = $con->getArray("select employer from pccmain.patient_info where patient_id = '$_REQUEST[pid]';");
		}
	...
	if($_REQUEST['pid'] == '123525' || $_REQUEST['pid'] == '95032' || $_REQUEST['pid'] == '63117') { $paper = "FOLIO"; } else { $paper = "LETTER"; }

	 /* END OF SQL QUERIES */

$mpdf=new mPDF('win-1252',$paper,'','',10,10,30,10,5,5);
			...
			<td>
				...
				<table width="100%" cellspacing=0 cellpadding=0>
					<tr>
						...
						<td width=23% class="td-1">Date &nbsp;<b>'.$resultDatex.'</b></td>
					</tr>
				</table>
				<table width="100%" cellspacing=0 cellpadding=0>
					<tr>
				...
				<table width="100%" cellspacing=0 cellpadding=0>
					<tr>
						<td width=55% class="td-1">Name of Company &nbsp;<b>'.$cname.'</b></td>
