...
/* MYSQL QUERIES SECTION */
	...
	//$outerQuery = $con->dbquery("SELECT a.pid, CONCAT(b.lname,', ',b.fname,' ',b.mname) AS pname, b.birthdate, date_format(b.birthdate,'%d %b %Y') as bday, if(b.gender='M','Male','Female') as gender, b.street, b.brgy, b.city, b.province, b.gender as xgender, a.barcode, DATE_FORMAT(c.extractdate,'%m/%d/%Y') AS extractdate FROM cso_details a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN lab_samples c on a.pid = c.pid WHERE a.cso_no = '$_REQUEST[so_no]' and a.barcode != 'N' and c.extractdate between '$dtf' and '$dt2' GROUP BY a.pid ORDER BY b.lname desc, b.fname, b.mname;");
	$outerQuery = $con->dbquery("SELECT a.pid, CONCAT(b.lname,', ',b.fname,' ',b.mname) AS pname, b.birthdate, DATE_FORMAT(b.birthdate,'%d %b %Y') AS bday, IF(b.gender='M','Male','Female') AS gender, b.street, b.brgy, b.city, b.province, b.gender AS xgender, a.barcode, DATE_FORMAT(c.extractdate,'%m/%d/%Y') AS extractdate, b.employer FROM cso_details a LEFT JOIN pccmain.patient_info b ON a.pid = b.patient_id LEFT JOIN lab_samples c ON a.pid = c.pid WHERE a.cso_no = '6' AND a.barcode != 'N' AND c.extractdate BETWEEN '$dtf' AND '$dt2' AND b.employer= 'ADVANCED WORLD SOLUTIONS, INC.' GROUP BY a.pid ORDER BY b.lname DESC, b.fname, b.mname;");
