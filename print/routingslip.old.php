<?php
    ...
    $_ihead = $con->getArray("SELECT *, LPAD(a.cso_no,6,0) AS sono, DATE_FORMAT(a.cso_date,'%m/%d/%Y') AS d8, b.pid FROM cso_header a left join cso_details b on a.cso_no = b.cso_no and a.trace_no = b.trace_no where a.cso_no = '$_REQUEST[cso_no]' and a.branch = '1' and b.pid= '$_REQUEST[pid]';");
    //$_p = $con->getArray("SELECT YEAR(CURDATE()) - YEAR(birthdate) as age, DATE_FORMAT(birthdate,'%M %d, %Y') AS bday, IF(gender='M','Male','Female') AS gender, if(a.employer='','WALK-IN',a.employer) as employer, a.cstat FROM pccmain.patient_info a WHERE a.patient_id = '$_ihead[pid]';");
