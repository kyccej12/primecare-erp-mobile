<?php
	...
	if($_GET['shift'] != '') {
		...
		if($_GET['shift'] == 1) {
			...
			$fs .= " AND b.barcode = 'Y' AND b.processed_on BETWEEN '$date 14:01:00' and '$date 22:00:00' ";
		}
	...
	$sql = $con->dbquery("SELECT a.cso_no, DATE_FORMAT(a.cso_date,'%m/%d/%Y') AS date_availed, CONCAT(DATE_FORMAT(a.from,'%m/%d/%Y'),' - ',DATE_FORMAT(a.until,'%m/%d/%Y')) AS scheduled_date, DATE_FORMAT(a.cso_date, '%m/%d/%Y') AS cso_date, a.customer_name, b.pid, CONCAT(c.lname,', ',c.fname, ', ',c.mname) AS pname, IF(c.gender='M','MALE','FEMALE') AS gender, DATE_FORMAT(c.birthdate,'%d %b %Y') AS bday, c.birthdate, ROUND(DATEDIFF(a.cso_date,c.birthdate)/364.25) AS age, d.lotno, d.procedure, d.code, e.result_stat, date_format(e.result_date,'%m/%d/%Y') as result_date, CONCAT(f.fullname,' ,',f.prefix) AS consultant, g.fullname AS encoder FROM cso_header a LEFT JOIN cso_details b ON a.trace_no = b.trace_no LEFT JOIN pccmain.patient_info c ON b.pid = c.patient_id LEFT JOIN lab_samples d ON c.patient_id = d.pid LEFT JOIN lab_descriptive e ON d.pid = e.pid LEFT JOIN options_doctors f ON e.consultant = f.id LEFT JOIN user_info g ON e.verified_by = g.emp_id WHERE d.procedure LIKE '%chest%' AND d.extracted != 'N' AND b.cso_no= '$_REQUEST[so_no]' $fs GROUP BY d.pid ORDER BY b.pname;");
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue("K12","CODE");
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue("L12","COMPANY NAME");
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue("M12","CONSULTANT");
	...
	while($data = $sql->fetch_array()) {
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(11,$row,html_entity_decode($data['customer_name']));
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(12,$row,html_entity_decode($data['consultant']));
	...
	// Rename worksheet
	$objPHPExcel->getActiveSheet()->setTitle("Mobile X-Ray Log Book");
	
	// Set active sheet index to the first sheet, so Excel opens this as the first sheet
	$objPHPExcel->setActiveSheetIndex(0);
			
	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	header('Content-Disposition: attachment;filename="mobile-xray-logbook.xlsx"');
